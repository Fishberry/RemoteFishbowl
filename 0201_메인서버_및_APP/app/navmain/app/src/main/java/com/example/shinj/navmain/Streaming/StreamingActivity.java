...
//        final ProgressBar progressBarTemperature = (ProgressBar) findViewById(R.id.progresBarTemper);
        progressBarTemperature = (ProgressBar) findViewById(R.id.progresBarTemper);
            ...
            public void run() {
                ...
                while (true) {
                    try {
                        ...
                        socket.on("resTimerFeed", resTimerFeed);
            ...
            public void run() {
                while (true) {
                    try {
                        ...
                        socket.on(Socket.EVENT_CONNECT, onConnect);
                        socket.on("resTimerWater", resTimerWater);
...
//                        });
    ...
    private Emitter.Listener resTimerFeed = new Emitter.Listener() {
        @Override
        public void call(Object... objects) {
            runOnUiThread(()-> {
                if (!(objects[0].toString().equals("0")) || !(objects[1].toString().equals("0"))) {
    ...
    private Emitter.Listener resTimerWater = new Emitter.Listener() {
        @Override
        public void call(Object... objects) {
            runOnUiThread(new Runnable() {
                @Override
    ...
    private Emitter.Listener serverMsg = new Emitter.Listener() {
        @Override
        public void call(Object... objects) {
            runOnUiThread(new Runnable() {
                @Override
